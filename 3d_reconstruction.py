...
def get_relative_position(holographic_info_dict, left_camera, right_camera):
    ...
    R12 = left_camera_info["R"]
    T12 = left_camera_info["T"]
    P1 = np.dot(camera_matrix_left, np.hstack((R12, T12)))
...
if __name__ == "__main__":
    ...
    # [4] Let's select several pairs of cameras to get the corresponding world coordinates
    # ref_camera_info = holographic_info_dict["camera_11_left"]
    # ref_camera_info["R"] = np.eye(3)
    # ref_camera_info["T"] = np.zeros((3, 1))
    # R_left_right, T_left_right, corresponding_targets_list_left_right, target_point_world_left_right = get_relative_position(holographic_info_dict, "camera_11_left", "camera_11_right")
    # R_11_71, T_11_71, corresponding_targets_list_11_71, target_point_world_11_71 = get_relative_position(holographic_info_dict, "camera_11_left", "camera_71")
    # R_11_72, T_11_72, corresponding_targets_list_11_72, target_point_world_11_72 = get_relative_position(holographic_info_dict, "camera_11_left", "camera_72")
    #
    ...
    word_points_dict = {}
    word_points_dict[grggg_key] = grggg_coordinate
    ...
    for camera_key in holographic_info_dict.keys():
        ...
        P = np.dot(curr_camera_matrix, np.hstack((R_camera, tvec)))

        # Compute the inverse of the projection matrix
        P_inv_proj = np.linalg.inv(P.T @ P)

        # Calculate the 3D points. We assume all points lie on the plane Z=1 in the world frame
        # point_4d_hom = np.dot(P_inv_proj, P.T @ homogeneous_imgpoints_ref.T)
        # point_3d = point_4d_hom / np.tile(point_4d_hom[-1, :], (4, 1))
        # inv_target_point_world = point_3d[:3, :].T
        # error = cv2.norm(curr_target_point_world, inv_target_point_world, cv2.NORM_L2) / len(curr_target_point_world)
        # print("Inverse Projection error: ", error)

        homogeneous_imgpoints_ref = np.hstack((curr_target_point_image, np.ones((curr_target_point_image.shape[0], 1))))
        normalized_point = np.linalg.inv(curr_camera_matrix) @ homogeneous_imgpoints_ref.T
        camera_point = normalized_point / np.tile(normalized_point[-1, :], (3, 1))

        word_point = R_camera @ camera_point + tvec

        error = cv2.norm(curr_target_point_world, word_point, cv2.NORM_L2) / len(curr_target_point_world)
        print("Inverse Projection error: ", error)
